...
namespace absFunc {
    ...
    // turns a single operation function f: In -> Out into a vectorized function f: In[] -> Out[]
    template <typename In, typename Out>
    function<vector<Out>(const vector<In>&)> vectorize(function<Out(const In&)> f) {
        return [f] (const vector<In>& input) {
            return map_f<In, Out>(input, f);
        };
    }
