...
// this is a HELPER function - do NOT call this function outside of this file (parseArguments.h)
string parse::parseArgumentHelper(const string& command, const string& sep) {
    int indexSep = strUtil::positionOfFirstOccurrence(command, sep);
    string argumentPortion = command.substr(indexSep + sep.size());
    return argumentPortion;
}
...
// parses the position-th argument of the command
string parse::parseArgument(const string& command, int position, const string& sep) {
    if (position > numArguments(command, sep)) {
        throw invalid_argument("position (" + to_string(position) + ") > # of arguments (" + to_string(numArguments(command, sep)) + ")");
    }
    if (position == 0) {
    ...
    string processedCommand = removeSubstrings(command, sep, position - 1);
    string argument = parseArgumentHelper(processedCommand, sep);
    return removeAllAfterSubstring(argument, sep);
}
...
// parses the position-th numerical argument of the command
// do NOT use to parse non-numerical arguments
double parse::parseNumericalArgument(const string& command, int position, const string& sep) {
    return stod(parseArgument(command, position, sep));
}
...
// parses the position-th boolean argument of the command
// do NOT use to parse non-boolean arguments
bool parse::parseBooleanArgument(const string& command, int position, const string& sep) {
    return parseBool(parseArgument(command, position, sep));
}
...
// parse 1 until the end returns "arg1 arg2 arg3 arg4 arg5"
// parse 3 until the end returns "arg3 arg4 arg5"
string parse::parseArgumentUntilEnd(const string& command, int position, const string& sep) {
    if (position > numArguments(command, sep)) {
        throw invalid_argument("position (" + to_string(position) + ") > # of arguments (" + to_string(numArguments(command, sep)) + ")");
    }
    if (position == 0) {
...
int parse::numArguments(const string& command, char sep, bool includeCommandName) {
    return numOccurrences(command, sep) + includeCommandName;
}

// returns the number of arguments in the command
int parse::numArguments(const string& command, const string& sep, bool includeCommandName) {
    return numOccurrencesNoOverlap(command, sep) + includeCommandName;
}
...
// checks if the command's name is commandName
bool parse::commandIs(const string& command, const string& commandName, const string& sep) {
    if (numArguments(command, sep) == 0) {
        return command == commandName;
    } else {
        return beginsWith(command, commandName + sep);
...
// returns a trimmed version of the command consisting only of the command name and the first n arguments
string parse::commandPlusNargs(const string& command, int n, const string& sep) {
    if (n > numArguments(command, sep)) {
        throw invalid_argument("n (" + to_string(n) + ") > # of arguments (" + to_string(numArguments(command, sep)) + ")");
    }
    if (n == 0) {
    ...
    vector<string> allArguments = parseAllArguments(command, true, sep);
    return strUtil::join(vecUtil::subvector(allArguments, 0, n), sep);
}
...
// returns the command's name
string parse::getCommandName(const string& command, const string& sep) {
    return strUtil::removeAllAfterSubstring(command, sep);
}
...
// parses all arguments of the command, returning a list of arguments
vector<string> parse::parseAllArguments(const string& command, bool includeCommandName, const string& sep) {
    vector<string> allArguments = strUtil::split(command, sep);
    return includeCommandName ? allArguments : vecUtil::subvector(allArguments, 1, allArguments.size() - 1);
}
...
// parses all numerical arguments of the command, returning a list of arguments
// note that if includeCommandName is true, then the "command name" needs to be numerical as well (i.e. not a traditional command name)
vector<double> parse::parseAllNumericalArguments(const string& command, bool includeCommandName, const string& sep) {
    return absFunc::map<string, double>(parseAllArguments(command, includeCommandName, sep), [] (const string& arg) {
        return stod(arg);
    });
}
...
// parses all boolean arguments of the command, returning a list of arguments
// note that if includeCommandName is true, then the "command name" needs to be boolean as well (i.e. not a traditional command name)
vector<bool> parse::parseAllBooleanArguments(const string& command, bool includeCommandName, const string& sep) {
    return absFunc::map<string, bool>(parseAllArguments(command, includeCommandName, sep), [] (const string& arg) {
